...
## 제외 확정 1개 (사용자 결정 2026-07-07)

| 노드 | 애매성 | 결정 |
|---|---|---|
| **LLM-as-a-judge** | "LLM을 심판으로" **범주어**인가, **명명된 평가 패러다임**(Agent-as-a-Judge가 확장)인가 — 50/50. "as-a-judge" 표현의 범주 성격이 강함. | **제외 확정** |

**사용자 결정**: 명명 명확성이 50/50이라 "명확한 named 방법" 기준에 미달 → 제외가 일관(TIR·self-reflection·GNN
제외와 동일 결). metric gaming 안 하는 정직성 유지(TP+2를 위해 애매 노드 추가하지 않음). **lexicon 무변경**
(원래 미추가 상태 유지). 확정 채점 수치는 그대로(LLM-as-a-judge 미반영 기준).
