...
"""PHASE A4/A5 집계 — 서브에이전트 결과(ma_res_0..7.json)를 모아:
  A4: keep_drop_eval.csv 매니페스트 + papers.json 에 multiagent_b3 그룹 append(포맷 보존)
...
# ── 2. keep_drop_eval.csv
with (EXP/"keep_drop_eval.csv").open("w", newline="") as f:
    w = csv.writer(f)
...
print(f"keep_drop_eval.csv 기록({len(res)}행)")
...
data["_meta"]["_draft_note_eval"] = (f"{GROUP} 초안 {added}편 append(2026-07-07, D1~D4). "
