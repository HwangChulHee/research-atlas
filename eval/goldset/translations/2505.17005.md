...
- RAG: "outperforms previous RAG" 베이스라인 + RAG(Gao 2024). 4회.
- 제외(lineage-only): MCTS(부품 알고리즘 — "MCTS framework (Sun 2025) ... incurs significant overhead"로 단점만 짚음, 방법적 후예 아님 → 루브릭 부품 제외), Qwen-2.5-7B-Instruct(실험 substrate일 뿐 "딛/보강" 아님), R1-Searcher·Search-R1(본문 미호명, 인용·URL뿐), SFT(일반 학습방식+자기 단계로도 사용).
