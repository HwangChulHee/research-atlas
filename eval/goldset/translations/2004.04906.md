...
- ORQA: "before ORQA (Lee 2019), which proposes ICT... ORQA demonstrates dense can outperform BM25... it also suffers from two weaknesses (ICT pretraining 비쌈, context encoder fine-tune 안 됨)" → DPR이 두 흠 정면 진단+해결(추가 사전학습 없이 양 encoder fine-tune) = 강한 improve-upon. 직계 선행.
- BERT: "leveraging the now standard BERT pretrained model + dual-encoder로 dense 검색" → BERT 인코더를 검색용으로 *적응*해 방법을 세움 = lineage 포함. (substrate 잣대: 적응 대상이면 포함, 단순 실험백본이면 제외 — ColBERT서 확정한 기준 소급 적용.)
- 제외 BM25·TF-IDF: DPR이 능가하는 sparse baseline, dense는 대안이지 descend 아님 → baseline/contrast.
- 제외: dual-encoder/Bromley1994(저자-연도+보통명사 아키텍처 부품), ICT(ORQA 기법, DPR은 안 씀-대조), MIPS(효율 도구), Chen2017·machine reader(저자-연도+보통명사), NaturalQuestions·SQuAD(벤치), RAG(본문 미등장), DPR(self).
- 지형: RAG의 검색기 쪽 뿌리(dense 검색 표준). REALM과 함께 2020 retrieval-augmented 뿌리.
