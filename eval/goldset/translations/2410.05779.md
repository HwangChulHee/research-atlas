...
- RAG: "RAG systems... existing RAG systems have key limitations: flat data representations... lack contextual awareness" → LightRAG가 RAG의 flat 표현 흠 진단+그래프 통합으로 개선 = improve-upon. 패러다임 뿌리.
- 경계 제외 GraphRAG: LightRAG는 분명 GraphRAG 계열이나 abstract+intro 본문에 "GraphRAG" 이름이 안 나옴("graph structures into text indexing" 일반 서술, "graph-empowered RAG" 보통명사). 본문 고유명사 기준상 제외(①). ← abstract-only 위상 한계의 전형(형제 GraphRAG가 related work에 있어 누락, 메모리의 그 케이스).
- 제외: chunking(Lyu2024 저자-연도), dual-level retrieval·incremental update(self), flat data representation(대조 개념 보통명사), 인용 전부 저자-연도(Sudhi·Es·Salemi), graph+vector(보통명사 구성요소), self.
- 지형: RAG의 그래프 가지(GraphRAG·HippoRAG·HyperGraphRAG 형제). 효율·증분 특화(GraphRAG 비싼 community 경량 대안). RAG-dominant 패턴=abstract-only 구조적 결과.
