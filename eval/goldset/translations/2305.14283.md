...
- RAG: "retrieve-then-read framework (Lewis 2020b; Karpukhin 2020)... regarded as standard" → RRR은 retrieve-then-read(RAG 파이프라인)에 rewrite 단계를 더한 확장, RAG 패러다임 위.
- 경계 제외 REPLUG(Shi 2023): "train dense retriever to cater to frozen LM (Shi 2023)" LLM-oriented adaptation 한 갈래로 언급. RRR은 query 적응, REPLUG는 retriever 적응 — 다른 부분이라 대조/관련연구지 딛은 것 아님 + 저자-연도(이름 본문 미등장 ①).
- 경계 제외 ReAct(Yao2023)·DSP(Khattab2022): "interactions between retriever and reader" 다른 갈래, RRR이 "multiple rounds 불필요"로 대조. 저자-연도.
- 제외 T5-large: rewriter 자리에 넣어 RL 훈련한 백본 = substrate(DPR의 BERT처럼 "방법 본질 적응"이 아니라 OTC의 Qwen 결).
- 제외: ChatGPT·Vicuna-13B(frozen reader substrate), 웹 검색엔진(도구), HotpotQA·AmbigNQ·PopQA·MMLU(벤치), RL(훈련법 보통명사), self.
- 지형: RAG의 query rewriting 가지(DeepRetrieval·s3 이웃). reader 성능을 보상으로=s3 GBR 유사 초기형.
