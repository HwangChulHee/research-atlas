...
- RAG 아닌 agent memory 논문. 기존 메모리(raw trajectory/workflow)를 개선하나 그 선행들이 범주 묘사 + 저자-연도 인용(Kagaya/Kong/Fang 2024-25)이라 named 조상 없음.
- 제외: raw trajectory memory·workflow memory(Fig1 비교지만 메모리 유형 범주 라벨, named 시스템 아님 — ①⑤), agent memory(분야 범주+저자-연도), WebArena·Mind2Web·SWE-Bench-Verified(벤치), ReasoningBank·MaTTS(self).
- 경계 제외 test-time scaling: MaTTS가 그 위에 서나 RAG 같은 명명 패러다임 아니라 일반 개념(단일 origin 없는 우산) → TIR과 같은 결, lexicon 거름 대상.
- 채점 의미: 4번째 empty(이유 다름 — agent-memory, 선행이 범주+저자-연도). false-positive 테스트.
- 지형: agent memory/self-evolution 가지(RAGEN 이웃), 미국 주도(Google/UIUC).
