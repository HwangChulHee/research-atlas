...
- CoT: "chain-of-thought (CoT) prompting (Wei et al., 2022b)" 명명 + 사람 시범 의존 한계를 순수 RL로 넘어섬(improve upon).
- DeepSeek-V3-Base: "we build upon DeepSeek-V3-Base" — 명시적 베이스 모델.
- 제외: DeepSeek-R1-Zero/DeepSeek-R1(이 논문이 스스로 정의 = self-defined), GRPO(사용한 RL 알고리즘), SFT/RL(일반 학습방식·보통명사).
