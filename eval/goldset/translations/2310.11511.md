...
- RAG: "RAG methods (Lewis 2020; Guu 2020) augment input... However retrieve indiscriminately" → Self-RAG가 RAG 무차별 검색 흠 진단+적응검색·자기비평으로 개선 = improve-upon. RAG 패러다임 위 직계.
- 제외 REPLUG(Shi2023): "introduce off-topic passages (Shi2023)" 무차별 검색 문제 근거로 인용, 저자-연도(딛은 것 아님 ①).
- 제외 control tokens(Lu2022·Keskar2019)·reward models in RL(Ziegler2019·Ouyang2022): reflection token이 닮았으나 "inspired by" 영감+저자-연도.
- 제외 GPT-4: critic 라벨(reflection token 데이터) 생성에 쓴 도구/substrate.
- 제외: ChatGPT·Llama2-chat·Alpaca(능가 baseline), REALM/Guu2020(RAG 옆 인용 저자-연도), beam search(디코딩 부품), six tasks(벤치), self.
- 지형: RAG의 자기성찰/적응검색 가지(CRAG·DeepRAG 이웃). s3에서 SFT 단계로 언급된 논문.
