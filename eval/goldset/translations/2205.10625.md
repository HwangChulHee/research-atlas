...
- Chain-of-Thought: "CoT has a key limitation—performs poorly on tasks harder than demonstrations... To tackle such easy-to-hard generalization, we propose least-to-most" → CoT의 흠(easy-to-hard 실패) 진단+분해로 해결 = improve-upon. CoT 위 직계.
- 경계 제외 self-consistency: "when further integrated with self-consistency decoding" 언급되나 CoT 설명 중 곁들인 맥락이지 Least-to-Most가 확장하는 대상 아님(저자-연도 Wang2022b).
- 제외: few-shot prompting/natural language rationales(CoT 설명 맥락, 보통명사+저자-연도 Ling/Cobbe/Brown), GPT-3 code-davinci-002(적용·시연 실험 모델 substrate), SCAN·neuro-symbolic 모델(벤치/대조), educational psychology Libby2008(이름 차용 영감), RAG(본문 미등장), self.
- 지형: CoT 계열, easy-to-hard를 분해로 보완. GlobalRAG의 subgoal 분해가 이 후예.
