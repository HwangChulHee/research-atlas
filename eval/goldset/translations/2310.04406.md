...
- ReAct: "LATS synergizes reasoning, acting, planning by expanding ReAct (Yao 2023b) into a search over combinatorial space" + "doubles the performance of ReAct" → ReAct를 트리 탐색으로 직접 확장 = extends. 명백한 직계.
- 경계 제외 self-consistency·self-refinement: "value function incorporates heuristics such as self-refinement and self-consistency" → 가치함수에 녹인 부품 휴리스틱(MCTS/PPO 차용 결). self-consistency는 named·골든셋에 있으나 부품 차용이라 제외.
- 경계 제외 Reflexion/self-reflection: "self-reflections" 컴포넌트 차용하나 Shinn2023 저자-연도(이름 미등장)+부품(①).
- 제외 search-guided LM(Xie2023·Yao2023a=ToT·Hao2023): 대조·개선하는 다른 갈래지만 전부 저자-연도, ToT도 이름 미등장(①).
- 제외 MCTS: LATS 핵심이나 부품 알고리즘(루브릭 명시 제외, PPO/GRPO/MCTS).
- 제외: GPT-4·GPT-3.5(적용·평가 substrate), HumanEval·HotpotQA·WebShop(벤치), RL(보통명사), RAG(본문 미등장), self.
- 지형: 트리 탐색 추론 가지 허브. ReAct 직계 확장+self-consistency/Reflexion 흡수. 후예(I-MCTS·MCTS-Judge)는 효율 가지로(2026 최전선=rollout 비용 줄이기).
