...
- RAG: "RAG (Lewis 2020)... vector RAG works well for local queries... However do not support sensemaking" → GraphRAG가 vector RAG의 전역 질문 흠 진단+그래프·커뮤니티 요약으로 개선 = improve-upon. 패러다임 뿌리. ("vector RAG"=RAG 별칭, 별도 노드 아님.)
- 경계 제외 QFS(query-focused summarization): "this is inherently a QFS task... Prior QFS methods do not scale" — RAG와 합치려는 다른 갈래지만 기법 범주(보통명사)+선행 저자-연도(Baumel2018 등), 특정 named QFS 기법을 딛는 게 아니라 개념적 결합(TIR/test-time-scaling 우산 결) → narrow 제외.
- 제외: GPT·Llama·Gemini(sensemaking LLM 예시+GPT-4 평가 substrate), LLM-as-a-judge(Zheng2024 평가 도구 저자-연도), LangChain·LlamaIndex·Neo4j(배포 라이브러리 도구), map-reduce·community detection(부품), self.
- 지형: RAG의 구조화 인덱싱/그래프 가지 허브. RAPTOR(트리) 형제. KG-R1·MARAG-R1이 대조한 그 GraphRAG. HippoRAG·LightRAG·HyperGraphRAG 조상.
