...
- Zero-shot-CoT: "To address the issue of Zero-shot-CoT caused by missing reasoning steps, we propose Plan-and-Solve" → PS가 "Let's think step by step"을 직접 교체·확장, 단계 누락 흠 진단+해결 = improve-upon. 가장 직접적 부모.
- Chain-of-Thought: few-shot CoT(Wei 2022), Zero-shot-CoT의 조상이자 PS의 패러다임 뿌리. 직접 부모+패러다임 뿌리 둘 다(KG-R1의 KG-RAG+RAG 선례 일관).
- 제외: Zero-shot-Program-of-Thought/PoT(Chen 2022, "comparable to or exceeds" 점수비교 baseline), GPT-3(적용·평가 실험 모델 substrate, API frozen), PTMs/BERT·RoBERTa("unlike previous PTMs" 대조), AQuA·GSM8K·SVAMP 등 10개(벤치), few-shot/zero-shot learning(보통명사), RAG(본문 미등장), PS·PS+(self).
- 지형: CoT 직계, 계획/분해 가지(Least-to-Most·GlobalRAG와 한 가지). PS는 zero-shot 프롬프트 한 줄로 계획 유도.
