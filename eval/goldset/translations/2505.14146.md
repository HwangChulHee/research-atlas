...
- RAG: "Gain Beyond RAG", s3가 딛고 진전시키는 패러다임. lineage(패러다임 조상).
- Search-R1: "tight entanglement makes it difficult to isolate retrieval improvements... EM is brittle. This motivates a shift toward a modular framework" → s3가 엉킴 진단·해결 = improve-upon.
- DeepRetrieval: "NDCG... disconnected from downstream answer quality" → s3의 GBR이 이를 해결 = improve-upon.
- 경계 제외(lean): DeepSeek-R1-Zero("Building on this idea, DeepRetrieval"의 주어가 s3 아님, RLVR period 할아버지), Self-RAG(Fig2 진화도의 SFT phase지 s3 직계 아님).
- 명백 제외: Search-o1·IRCoT·CoT(Fig1 baseline/Active RAG phase, descent 명시 없음), BM25·E5(검색 부품), NDCG·EM(지표), RLVR(보통명사), Claude-3-Haiku(실험용 frozen 생성기=도구), s3·GBR(self).
- 메모: lineage-only 첫 다중후보 케이스. "진단+직접해결=improve-upon" vs "점수만 높음=baseline"의 잣대 적용(Search-R1/DeepRetrieval은 흠을 명시 진단하고 s3가 고침 → 계보).
