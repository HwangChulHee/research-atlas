...
- RAG: "retrieval augmentation (Lewis 2020)... existing approaches retrieve only a few short contiguous chunks" → RAPTOR가 RAG 청크 지엽성 흠 진단+재귀 요약 트리로 개선 = improve-upon. 패러다임 뿌리.
- 제외 open-domain QA(Chen2017·Yu2018): "pioneered in open domain QA" 검색 인덱싱 시초로 언급, 저자-연도(①).
- 제외 UnifiedQA·GPT-3·GPT-4: "coupling RAPTOR with GPT-4" — RAPTOR는 검색이고 LLM은 frozen reader, 결합·평가한 substrate.
- 제외: clustering·summarization(RAPTOR가 쓰는 부품 보통명사), NarrativeQA·QASPER·QuALITY(벤치), REALM/Izacard(RAG 옆 저자-연도), self.
- 지형: RAG의 구조화 인덱싱 가지(GraphRAG 이웃 — 트리 vs 그래프, 둘 다 청크 검색 넘어 구조 만들기). 사용자 캡스톤 후보②(대규모 문서처리).
