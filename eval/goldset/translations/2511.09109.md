...
- RAG: "Retrieval-augmented generation (RAG) (Lewis et al. 2020)" 패러다임 뿌리.
- Search-R1: 한 문단 통째로 진단("Search-R1... outcome-based reward... lacks feedback for intermediate steps... reward hacking") → Bi-RAR가 step 단위 양방향 감독으로 해결 = 강한 improve-upon(GlobalRAG의 Search-R1과 동급).
- 경계 제외 Kolmogorov complexity·information distance: Bi-RAR metric의 이론적 토대로 차용하나, RAG/추론 계열 선행 방법이 아니라 정보이론 수학 개념/도구(MDP·RL 같은 형식) → 제외(formalism/tool).
- 제외: GRPO·multi-objective RL·weight-space interpolation·RL(부품·방법 보통명사), Qwen2.5-3B(백본 substrate ④), 7 QA benchmarks(벤치), forward/backward reasoning(Chen 2024 저자-연도+인지 영감), GPT-4·Gemini(저자-연도), Bi-RAR(self).
- 지형: process-reward 클러스터(GlobalRAG·HiPRAG 이웃), 양방향 정보거리 정량화가 특징.
