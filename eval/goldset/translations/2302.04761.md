...
- tool use 가문의 뿌리. 자기지도로 "도움되는 API 호출"을 모델이 스스로 선별·학습. 토대가 전부 보통명사+저자-연도라 named 조상 없음.
- 제외: in-context learning(Brown2020=GPT-3 보통명사+저자-연도), dataset generation 선행(Schick&Schütze2021·Honovich2022·Wang2022 저자-연도), 기존 tool use(Komeili2022·Thoppilan2022·Gao2022·Parisi2022=TALM, 흠 지적하나 전부 저자-연도·이름 미등장 ①), GPT-J(Wang&Komatsuzaki2021, finetune 대상 백본 substrate), GPT-3(능가한 baseline), CoT/Wei2022(맥락 언급), 계산기·QA·검색·번역·달력(사용 도구), RAG(본문 미등장), self.
- 채점 의미: 뿌리 empty(토대가 보통명사/저자-연도). 파이프라인이 GPT-J/GPT-3/in-context learning 환각 뱉을지 false-positive 테스트.
- 지형: tool use 뿌리. ReAct의 Act·OTC의 TIR·MARAG-R1 멀티툴이 후예. 도구 사용을 학습으로 내재화한 초기 형태.
