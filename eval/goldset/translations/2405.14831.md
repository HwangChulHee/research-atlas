...
- RAG: "RAG has become the de facto solution... However current RAG methods unable to integrate across passage boundaries" → HippoRAG가 RAG의 패시지 고립 흠 진단+KG/PPR로 개선 = improve-upon. 패러다임 뿌리.
- 경계 제외 IRCoT: "HippoRAG is 10-30× cheaper than IRCoT [78]... combined with IRCoT brings further gains" → IRCoT는 능가 baseline(반복검색)+결합 옵션이지, HippoRAG 방법(KG+PPR 단일검색)이 IRCoT에서 descend 안 함(독립 대안 메커니즘). "combined with"는 결합이지 딛음 아님 → Voyager의 ReAct·CRAG의 Self-RAG 제외 결.
- 제외 Personalized PageRank(PPR): HippoRAG가 쓰는 알고리즘 부품(PPO/GRPO/MCTS 결).
- 제외 hippocampal indexing theory(Teyler&Discenna): 영감 출처(신경과학 이론, 저자-연도).
- 제외: knowledge graph(보통명사 구성요소), MuSiQue·2Wiki·HotpotQA(벤치), model editing(대조 저자-연도), self.
- 지형: RAG의 그래프 가지(GraphRAG·LightRAG·HyperGraphRAG 형제). PPR 연상 특화 + 효율(IRCoT 대비 싸고 빠름) 강조.
