...
## 2026-06-23 — 사전 카드 폰트↑·상태설명 정리 + 지형도 기본 RAG 계보 + 잔여 UI

- 사전 카드 폰트 상향: 개념명 15→17, 근거 13→14.5, 논문/별칭/제안/버튼 13~13.5. 상태 설명(legend) 문구 한 줄로 축약 + 2열 grid·nowrap·ellipsis로 정렬(2줄 깨짐 해소).
- 지형도 기본 뷰 = **RAG 계보 고정**(전체 헤어볼 대신). 들어온 질의(run/collect) 없을 때 ready 후 lineageSets("RAG","both") 하이라이트 + 칩 "기본: RAG 계보". (임시 — 추후 기본 뷰 방식 재고.)
- #11 계보 방향 색 범례: 계보 강조 중 상단에 조상(파랑)·자손(주황)·기준(검정) 표시.
- #8 비용 버튼 구분: 재빌드/제안 새로고침에 .lex-op(accent 아웃라인)로 일반 편집과 시각 구분.
- web build 통과.

## 2026-06-23 — 지형도 재설계: 통합 도구 패널 + 풀하이트 디테일(초록) + 사전 추가폴리시

지형도 종합 개선(사용자: 검색·필터 통합·단순화·on/off, 노드 클릭 시 초록·한 면 패널):
- 좌상단 **통합 도구 패널**(.graph-tools): [🔍 검색·필터 ▾ 토글] + [⤢ 전체] 헤더, 펼치면 검색+필터(유형/분야/시점 셀렉트·안본것만·논문보기·초기화). 기본 접힘(깔끔). 기존 하단 컨트롤 바·우상단 fit 버튼 제거 → 한 곳으로 통합.
- 노드 클릭 디테일 패널을 **오른쪽 한 면 전체(풀하이트 도킹)**로. 관련 논문 **초록·도입 발췌** 표시: GET /api/paper/{id}(parsed/concepts.json) + getPaper, selected 변경 시 lazy fetch(논문노드=자기, 개념=첫 등장 논문), 로딩 스피너.
- 줌 라벨 임계 0.7.
- 사전: 페이지당 10, 카드 버튼/상태select 크게, 헤더 설명 1줄.
- 검증: /api/paper 200(초록 1800자)·404, web build 통과.
